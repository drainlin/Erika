...
napi_value NativeAudioOnlyTick(napi_env env, napi_callback_info info) {
  size_t argc = 1;
  napi_value args[1] = {};
  napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
  if (argc == 0) {
  ...
  return Int32(env, erika_presenter_audio_only_tick(player->presenter, &stats));
}
